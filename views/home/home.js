app.controller('homeCtrl', ['$scope', '$state', '$filter', '$cookies', function ($scope, $state, $filter, $cookies) {

$("#word1").html($filter('translate')('STUDY_GROUP'));
$("#word2").html($filter('translate')('SOFTWARE_ENGINEERING'));
$("#word3").html($filter('translate')('MAS'));
...
var animation = function($) {
  ...
  function animateHeadline($headlines) {
    ...
    $headlines.each(function(){
      ...
      //trigger animation
      $scope.timeoutVar = setTimeout(function(){ hideWord( headline.find('.is-visible').eq(0) ) }, duration);
    });
  }
